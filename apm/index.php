...
<?php include 'header.php'; ?>
 ...
 <main class="bg-black text-white text-center py-24">
  ...
  <div class="whatsapp-icon" onclick="openWhatsApp()" style="cursor:pointer;">
...
?>
  <section class="bg-white py-20">
    <div class="max-w-6xl mx-auto px-6 text-center">
      ...
      <div class="relative overflow-hidden">
        <div id="review-carousel" class="flex space-x-6 overflow-x-hidden scroll-smooth snap-x snap-mandatory no-scrollbar">
          <?php if ($reviews->num_rows > 0): ?>
            <?php while ($row = $reviews->fetch_assoc()): ?>
              <div class="bg-gradient-to-r from-yellow-100 via-yellow-200 to-yellow-100 p-6 rounded-3xl shadow-lg hover:shadow-2xl transition flex-shrink-0 w-80 snap-center border border-yellow-300">
                <p class="text-gray-800 italic mb-4 text-lg font-serif leading-relaxed"><?= htmlspecialchars($row['review']) ?></p>
                <div class="flex flex-col items-center gap-2">
                  <div class="text-yellow-500 text-xl font-bold tracking-widest"><?= str_repeat('★', (int)$row['rating']) ?></div>
                  <p class="font-semibold text-gray-900 text-lg"><?= htmlspecialchars($row['name']) ?></p>
                </div>
              </div>
            <?php endwhile; ?>
          <?php else: ?>
            <p class="text-gray-700">No customer reviews available.</p>
          <?php endif; ?>
        </div>
      </div>
    </div>
